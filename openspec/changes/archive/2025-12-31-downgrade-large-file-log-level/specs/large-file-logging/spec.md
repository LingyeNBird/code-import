# large-file-logging Specification Delta

## Purpose
扩展 migration-logging 规范，定义大文件（>256M）导致 push 失败时的日志级别要求，确保日志级别准确反映问题的严重程度和可恢复性，避免误导用户。
...
## Cross-References
- **Extends**: migration-logging spec (添加 push 失败日志级别分类要求)
- **Related Change**: downgrade-large-file-log-level
- **Depends On**: N/A
- **Blocks**: N/A

## Implementation Notes
- 使用现有的 `IsExceededLimitError(output string) bool` 函数进行错误类型判断
- 该函数已在 `pkg/migrate/migrate.go:638` 被使用，经过验证
- 函数检查输出中是否包含 "exceeded limit" 字符串
- 修改只影响日志级别，不影响错误返回值和处理流程
